...
dependencies. Another option is to use a shortcode and SyntaxHighlighter
Evolved. 
...
Configure it however you wish. There is even an Emacs color theme too. Please
be sure read the [[https://github.com/syntaxhighlighter/syntaxhighlighter/wiki][SyntaxHighlighter Evolved WIKI]] in addition to using it's
download page, which at time can be a little confusing. 

To use this first you need to set the variable
~org2blog/wp-use-sourcecode-shortcode~ to ~t~.

SyntaxHighlighter Evolved does use optional source block attributes in its
HTML. For example when you want to highlight a particular line of code to
demonstrate some point you can specify that using a configuration parameter.
[[https://en.support.wordpress.com/code/posting-source-code/][This is how]] those parameters work and here is an example of how to add them to
your source block using an =#+ATTR_WP= line immediately preceding the
=#+BEGIN_SRC= line, e.g.
