...
[[https://www.gnu.org/software/emacs/][https://img.shields.io/badge/Emacs-%3E%3D%2026.2-brightgreen.svg]] [[https://orgmode.org/][https://img.shields.io/badge/Org%20mode-%3E%3D%209.1.9-brightgreen.svg]] [[https://wordpress.org/about/][https://img.shields.io/badge/WordPress-XML--RPC-brightgreen.svg]]
...
:end:
  -  [[#installation][Installation]]
    ...
    -  [[#using-a-package][Using A Package]]
      ...
      -  [[#using-this-projects-package][Using This Project's Package]]
...
Org2Blog currently requires Emacs 26.2 and uses Org 9.2.2 that comes with it.
...
- You can download it from here and install it using Emacs's package manager
- It is fun to build the package yourself then once you figure /that/ out
  translate it to the MELPA recipe. 

To use it:

- Download the [[https://github.com/org2blog/org2blog/releases][release]] you want given the version number appended to the file,
  which is a TAR file
  - If you want the in-development version get it [[file:/package][here]]
  - If you want to build it yourself for example for testing the package then
    see [[./build.sh][build.sh]].
- If you have already installed the package using another package manager,
  from MELPA and Use Package for example, delete the Org2Blog directory that
  got installed (probably under ~/.emacs.d/elpa and install this one instead.
- Run the command ~package-install-file~
- Choose the TAR file you just downloaded
...
If you are a developer or even just feel like exploring the code a little more
then you can run Org2Blog and it's supporting packages by downloading then
loading them yourself. Doing so will get you familiar with things like library
...
- ~owp-checkout-statement~ creates Git commands to get the code from GitHub
  into the directory you want it stored. Call it and enter the directory name.
  Copy and paste the commands from =*Messages*= into your Terminal to run them.
- ~owp-load-statement~ creates Elisp code to load the libraries. Copy them into
...
Now you've got everything you need to run Org2Blog against it's source code.
...
request. It is very fun and easy. 
...
- Subtree
  ...
  - =CATEGORY=
  - =POST_TAGS=
    - Though they are the same thing, due to technical reasons when tags
      appear under a Subtree they can't use the =TAGS= property like an Entry,
      they use =POST_TAGS= instead. Please take note of this when you convert
      an Entry post to a Subtree post.
  - =POSTID=
  - =PARENT=
  - =PERMALINK=
  - =DESCRIPTION= (aka excerpt)
...
Second Org2Blog this client to interact with WordPress.

=metaWeblog= is provided a package from Org2Blog to make it reusable for others
...
(setq gnutls-algorithm-priority "NORMAL:-VERS-TLS1.3")
    #+end_src
  - [[https://www.reddit.com/r/emacs/comments/cl2aw8/setting_setq_gnutlsalgorithmpriority/evw972x?utm_source=share&utm_medium=web2x][Via]] it seems that the bug is in =gnutls= and this solution works around it.
    Excellent. 
...
- Activities
  ...
  - Naming Things
    - Org2Blog's package was named ~org2blog/wp~ to prevent a namespace
      ...
      mode name and variable names retain the original namespace. That said
      the name of this software remains Org2Blog.
- Rules
